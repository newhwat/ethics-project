...
export default function hardwarePage({featuredProduct, hardware, user}) {
  return (
    <Page>
      <Header user={user}/>
      <Featured user={user?._id} product={featuredProduct}/>
      <Categories />
      <Listings user={user?._id} products={hardware}/>
    </Page>
  );
}
...
export async function getServerSideProps() {
  ...
  const hardware = await Product.find({topic: 'hardware'}, null, {sort: {'_id':-1}, limit:10});
