...
export default function linuxPage({featuredProduct, linux, user}) {
  return (
    <Page>
      <Header user={user}/>
      <Featured user={user?._id} product={featuredProduct}/>
      <Categories />
      <Listings user={user?._id} products={linux}/>
    </Page>
  );
}
...
export async function getServerSideProps() {
  ...
  const linux = await Product.find({topic: 'linux'}, null, {sort: {'_id':-1}, limit:10});
  const user = await User.findById(tempUserId);
  
  return {
    props: {
      featuredProduct: JSON.parse(JSON.stringify(featuredProduct)),
      linux: JSON.parse(JSON.stringify(linux)),
